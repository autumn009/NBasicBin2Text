...
namespace NBasicBin2Text
{
    ...
    "MKD$"
        ...
        private static int srcBytesPointer;

        private static int fgetc()
        {
            if (srcBytesPointer >= srcBytes.Count()) return -1;
            return srcBytes[srcBytesPointer++];
        }

        private static int peek(int offset)
        {
            int p = srcBytesPointer + offset;
            if (p >= srcBytes.Count()) return -1;
            return srcBytes[p];
        }
        ...
        static int Main(string[] args)
        {
            ...
            srcBytes = File.ReadAllBytes(srcFileName);
                // if CMT file, skip hedaer(10byte)+filename(6byte)
                if (cmtFlag) for (int i = 0; i < 16; i++) fgetc();
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    int lineNumberLow = fgetc();
                    int lineNumberHigh = fgetc();
